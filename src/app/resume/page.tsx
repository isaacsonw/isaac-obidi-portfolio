            ...
            <article key={project.id} className="break-inside-avoid">
              <div className="flex flex-wrap items-baseline justify-between gap-x-4">
                <h3 className="font-serif text-xl font-medium tracking-tight text-foreground">
                  ...
                  <span className="text-accent">{project.org ?? project.name}</span>
                </h3>
                <span className="font-mono text-xs text-faint">{project.years}</span>
              </div>
              ...
              <ul className="mt-2 space-y-1.5">
                {project.highlights.map((h) => (
                  <li key={h} className="flex gap-2.5 text-sm leading-relaxed text-muted">
                    <span aria-hidden="true" className="mt-[0.5rem] h-px w-3 flex-none bg-accent" />
                    {h}
                  </li>
                ))}
              </ul>
          ...
          <dl className="mt-4 space-y-2">
            {skills.map((group) => (
              <div key={group.label}>
                ...
                <dd className="text-sm text-foreground/85">{group.items.join(", ")}</dd>
              </div>
            ))}
          </dl>
        </section>
